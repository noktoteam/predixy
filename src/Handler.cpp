...
Handler::Handler(Proxy *p) : mStop(false),
                             mProxy(p),
...
void Handler::helloRequest(Request *req, const String &protocol)
{
    if (protocol.equal("3", true))
    {
        logError("h %d protocol 3 not supported", id());
    }
    logError("h %d protocol 2 assumed", id());
    ResponsePtr res = ResponseAlloc::create();
...
#define Scope(all, empty, header) ((all || empty || key.equal(header, true)) ? (buf = buf->fappend("# %s\n", header)) : nullptr)
